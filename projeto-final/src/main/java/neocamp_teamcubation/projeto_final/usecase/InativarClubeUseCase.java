...
import neocamp_teamcubation.projeto_final.domain.port.in.InativarClubePort;
...
public class InativarClubeUseCase implements InativarClubePort {

    private final ClubeRepositoryPort repo;

    @Override
    public void executar(Long id) {
        Clube c = repo.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Clube não encontrado"));
        if (Boolean.FALSE.equals(c.getAtivo())) return; // idempotente
        c.setAtivo(false);
        repo.save(c);
    }
}
